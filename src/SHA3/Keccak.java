...
public class Keccak implements HashFunction, Duplex {
	...
	int nTotalBits = 1600; // Default
	int r = 1024; // The rate of a sponge function. duplex bit-rate
	private static int c = 576; // The capacity of a sponge function.
	int d = 0; //
	byte[][] M;
	byte[] state = new byte[nTotalBits / 8];

	@Override
	public byte[] getHash(byte[] val) {

		byte[] s = new byte[nTotalBits / 8];
		...
		val = new byte[getC() / 16];
		byte[] z = new byte[0];

		while (z.length < getC() / 16) {
	...
	byte[][][] chi(byte[][][] a) {
		...
		for (int x = 0; x < 5; x++)
			for (int y = 0; y < 5; y++) {
				int x_aux1 = (x + 1) % 5;
				int x_aux2 = (x + 2) % 5;
				A[x][y] = Util.xor(a[x][y],
						Util.and(a[x_aux2][y], Util.not(a[x_aux1][y])));
			}
	...
	byte[][][] iota(byte[][][] a, int roundNumber) {
		a[0][0] = Util.xor(a[0][0],
				RoundConstants.getRoundConstant(roundNumber));
		return a;
	}
